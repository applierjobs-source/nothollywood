...
def public_gallery(limit: int = 60, hours: int = 168):
    """Public gallery of recent user-generated renders.

    Pulls the newest N renders from Supabase across all users, renders a
    minimal HTML page with playable <video> tags. Emails are masked. Zach's
    own renders are excluded so this reflects real user activity.

    Signed URLs are 1h TTL — the page needs a refresh after that to renew.
    """
    if not (SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY):
        return HTMLResponse("<h1>Gallery unavailable</h1>", status_code=503)
    import library as _lib
    ...
    # Filter, sign URLs, exclude Zach's test account
    cutoff = time.time() - hours * 3600
    EXCLUDED_EMAILS = {"zacharrow3@gmail.com"}
    items = []
    for row in rows:
        ...
        if ts < cutoff:
            continue
        email = email_by_id.get(row.get("user_id", ""), "")
    ...
    cards = []
    for it in items:
        prompt = esc(it["prompt"])[:280]
        cards.append(f'''
<article class="card">
  <video controls preload="metadata" playsinline src="{esc(it["video_url"])}"></video>
  <div class="meta">
    <p class="prompt">{prompt}</p>
    <p class="sub"><span>{esc(it["user"])}</span> · <span>{it["duration"]}s</span> · <span>{esc(it["resolution"])}</span></p>
  </div>
</article>''')

    body = "\n".join(cards) if cards else '<p style="opacity:.6;text-align:center;padding:80px 20px;">No renders yet.</p>'
...
<title>Community gallery — Not Hollywood</title>
<meta name="description" content="Real videos generated by Not Hollywood users.">
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  .meta {{ padding: 14px 16px 16px; flex: 1; display: flex; flex-direction: column; gap: 8px; }}
  .prompt {{
    margin: 0;
    font-size: 14px;
    color: #d4d4d4;
  ...
  .sub {{ margin: 0; font-size: 12px; color: #6a6a6a; }}
  footer {{
    text-align: center;
...
<body>
<header>
  <a href="/" class="home">← Not Hollywood</a>
  <h1>Community gallery</h1>
  <p class="tag">Real renders from real users. Updated live.</p>
</header>
<main>
{body}
</main>
<footer>
  <p>Want to make your own? <a href="/">Start a render →</a></p>
</footer>
</body>
</html>'''
